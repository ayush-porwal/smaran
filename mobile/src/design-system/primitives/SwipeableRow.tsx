...
function ActionPanel({ alignEnd, action }: { alignEnd: boolean; action: SwipeAction }) {
  const isDone = action.label.toLowerCase().includes('done');
  const isUndo = action.label.toLowerCase().includes('undo');
  const IconCmp = isDone ? CheckIcon : isUndo ? ArrowCounterClockwiseIcon : null;
  const labelColor =
    action.tone === 'accent'
      ? '$onAccent'
      : action.tone === 'danger'
...
export function SwipeableRow({ children, onRightSwipe, onLeftSwipe }: SwipeableRowProps) {
